...
class TrafficSimulator:
    ...
    def __init__(self, config,  output_path, is_record, record_stats, port=0):
        ...
        self.coop_gamma = config.get('coop_gamma')
        self.cur_episode = 0
        self.norms = {'wave': config.getfloat('norm_wave'),
                      'wait': config.getfloat('norm_wait')}
        self.clips = {'wave': config.getfloat('norm_wave'),
                      'wait': config.getfloat('norm_wait')}
        ...
        self._init_map()
        self._init_data(is_record, record_stats, output_path)
        self._init_test_seeds(test_seeds)
    ...
    def _init_data(self, is_record, record_stats, output_path):
    ...
    def _init_test_seeds(self, test_seeds):
    ...
    def _init_action_space(self):
        ...
        for node_name in self.node_names:
            ...
            phase_id = self._init_node_phase_id(node_name)
